...
{ ����Ԫ��һ���򵥵�HTML������Ԫ����֧�ָ��ӵİ����ű������ݵĽ�����ֻ�Ǽ򵥵Ľ���
  HTML �ı�ǩ������
  //��֪�ڽ���������ҳ��������
}
type
...
{$IFEND}
  ...
  {$IFDEF UNICODE}
  TQHtmlTagForEachProcA = reference to procedure(AHtmlTag: TQHtmlTag;
  ...
  public
    ...
    procedure ForEach(ACallback: TQHtmlTagForEachProc; ANest: Boolean;
      AParam: IntPtr); overload;
    {$IFDEF UNICODE}
    procedure ForEach(ACallback: TQHtmlTagForEachProcA;
      ANest: Boolean); overload;
    {$ENDIF}
...
begin
  ...
  FItems.CaseSensitive := False; // HTML �����ִ�Сд
end;
...
{$IFDEF UNICODE}
procedure TQHTMLTag.ForEach(ACallback: TQHtmlTagForEachProcA; ANest: Boolean);
var
  AContinue: Boolean;
...
var
  ...
  begin
    for I := 0 to AParent.Count - 1 do
    begin
      ACallback(AParent[I], AParam, AContinue);
      if not AContinue then
        Break;
      if ANest then
        DoEach(AParent[I]);
  ...
  begin
    ...
    begin
      ...
      begin
        Inc(s);
        SkipSpaceW(s);
        if StrCmpW(PQCharW(AChild.Name), 'img', True) = 0 then // img ���Ժ���
          continue;
        if StrCmpW(PQCharW(AChild.Name), 'input', True) = 0 then // img ���Ժ���
          continue;
        if StrCmpW(PQCharW(AChild.Name),'link',True)=0 then
          continue;
        if StrCmpW(PQCharW(AChild.Name),'meta',True)=0 then
          continue;
        if StrCmpW(PQCharW(AChild.Name), 'p', True) = 0 then // p ������/p��Ҳ����û��
        begin
          pl := s;
