...
type
  TFNDllMain = function(hinstDLL: HMODULE; Reason: Integer; Reserved: Pointer)
    : BOOL; stdcall;
  PMemModule = ^TMemModule;
  LPMemModule = PMemModule;
...
{$ENDIF}
  ...
  begin
    ...
    begin
      try
        if (not ret^.entrypoint(HMODULE(ret^.codeBase), DLL_PROCESS_ATTACH, 0))
        // if false
...
begin
  ...
  begin
    ...
    for i := 0 to exps^.NumberOfNames do
    begin
...
begin
  ...
  if pm^.initialized and Assigned(pm^.entrypoint) then
    pm.entrypoint(NativeUInt(pm^.codeBase), DLL_PROCESS_DETACH, 0);
