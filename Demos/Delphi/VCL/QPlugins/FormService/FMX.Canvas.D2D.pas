...
uses
  Winapi.Windows, Winapi.DXGI, Winapi.D3D10, Winapi.D2D1, Winapi.Wincodec,
  FMX.Graphics;
...
type
  ...
  public type
    ...
    TDirect3DSupport = (Undetermined, Direct3D10Legacy, Direct3D10,
      Direct3D10_1);
  private
  class var
  ...
  public
    ...
    class property SharedDWriteFactory: IDWriteFactory
      read GetSharedDWriteFactory;
  end;
...
uses
  Winapi.Messages, Winapi.MultiMon, Winapi.ActiveX, Winapi.UxTheme,
  Winapi.DxgiFormat, Winapi.DxgiType,
  Winapi.D3DCommon, Winapi.D3D10_1, System.Types, System.Classes,
  System.Character, System.SysUtils, System.Math,
  System.Math.Vectors, System.Win.ComObj, System.Generics.Collections,
  System.UITypes, System.UIConsts,
  System.Messaging, FMX.Consts, FMX.Types, FMX.Types3D, FMX.Platform.Win,
  FMX.Forms, FMX.TextLayout, FMX.Surfaces,
  FMX.Utils, FMX.Helpers.Win;

type
  TBitmapCodecWIC = class(TCustomBitmapCodecWIC)
  private
    function DecodeFrame(const Frame: IWICBitmapFrameDecode;
      const Bitmap: TBitmapSurface; const MaxSizeLimit: Cardinal = 0): Boolean;
  public
    ...
    function LoadFromStream(const AStream: TStream;
      const Bitmap: TBitmapSurface; const MaxSizeLimit: Cardinal)
      : Boolean; override;
    function SaveToStream(const AStream: TStream; const Bitmap: TBitmapSurface;
      const Extension: string; const SaveParams: PBitmapCodecSaveParams = nil)
      : Boolean; override;
  end;
  ...
  private const
    DefaultRenderTargetMode
      : TD2D1RenderTargetType = D2D1_RENDER_TARGET_TYPE_DEFAULT;
  private
  class var
  ...
  private
    ...
    function CreateD2DBitmapBrush(ABitmap: TBitmap; const AWrapMode: TWrapMode;
      const ARect: TRectF; const AOpacity: Single): ID2D1Brush;
    function CreateD2DGradientBrush(AGradient: TGradient; const ARect: TRectF;
      const AOpacity: Single): ID2D1Brush;
    procedure ApplyFill(const ABrush: TBrush; ARect: TRectF;
      const AOpacity: Single);
    procedure ApplyStroke(const AStroke: TStrokeBrush; ARect: TRectF;
      const AOpacity: Single);
    procedure ContextLostHandler(const Sender: TObject; const Msg: TMessage);
  ...
  protected
    ...
    { begin and }
    function DoBeginScene(const AClipRects: PClipRects = nil;
      AContextHandle: THandle = 0): Boolean; override;
    procedure DoEndScene; override;
    procedure DoFlush; override;
    { creation }
    constructor CreateFromWindow(const AParent: TWindowHandle;
    ...
    { Bitmaps }
    class function DoInitializeBitmap(const Width, Height: Integer;
      const Scale: Single; var PixelFormat: TPixelFormat): THandle; override;
    class procedure DoFinalizeBitmap(var Bitmap: THandle); override;
    class function DoMapBitmap(const Bitmap: THandle; const Access: TMapAccess;
      var Data: TBitmapData): Boolean; override;
    class procedure DoUnmapBitmap(const Bitmap: THandle;
      var Data: TBitmapData); override;
    { drawing }
    procedure DoSetMatrix(const M: TMatrix); override;
    procedure DoFillRect(const ARect: TRectF; const AOpacity: Single;
      const ABrush: TBrush); override;
    procedure DoFillPath(const APath: TPathData; const AOpacity: Single;
      const ABrush: TBrush); override;
    procedure DoFillEllipse(const ARect: TRectF; const AOpacity: Single;
      const ABrush: TBrush); override;
    procedure DoDrawBitmap(const ABitmap: TBitmap;
      const SrcRect, DstRect: TRectF; const AOpacity: Single;
      const HighSpeed: Boolean = False); override;
    procedure DoDrawLine(const APt1, APt2: TPointF; const AOpacity: Single;
      const ABrush: TStrokeBrush); override;
    procedure DoDrawRect(const ARect: TRectF; const AOpacity: Single;
      const ABrush: TStrokeBrush); override;
    procedure DoDrawPath(const APath: TPathData; const AOpacity: Single;
      const ABrush: TStrokeBrush); override;
    procedure DoDrawEllipse(const ARect: TRectF; const AOpacity: Single;
      const ABrush: TStrokeBrush); override;
  public
    destructor Destroy; override;
    procedure SetSize(const AWidth, AHeight: Integer); override;
    { caps }
    class function GetAttribute(const Value: TCanvasAttribute)
      : Integer; override;
    { buffer }
    procedure Clear(const Color: TAlphaColor); override;
    procedure ClearRect(const ARect: TRectF;
      const AColor: TAlphaColor = 0); override;
    { cliping }
    procedure IntersectClipRect(const ARect: TRectF); override;
    procedure ExcludeClipRect(const ARect: TRectF); override;
    { drawing }
    procedure MeasureLines(const ALines: TLineMetricInfo; const ARect: TRectF;
      const AText: string; const WordWrap: Boolean; const Flags: TFillTextFlags;
      const ATextAlign: TTextAlign;
      const AVTextAlign: TTextAlign = TTextAlign.Center); override;
    function PtInPath(const APoint: TPointF; const APath: TPathData)
      : Boolean; override;
  end;
  ...
  private type
    TDWriteFontDescriptor = record
      ...
      class function Create(const AFamilyName: string;
        const AWeight: TDWriteFontWeight; const AStyle: TDWriteFontStyle;
        const AStretch: TDWriteFontStretch): TDWriteFontDescriptor;
        static; inline;
    end;
  private
  class var
  ...
  private
    class function EnumFontFamExProc(const lpelfe: TLogFont;
      const lpntme: TNewTextMetric; FontType: DWORD; lParam: LPARAM): Integer;
      stdcall; static;
  private
...
begin
  ...
  begin
    if Failed(CoCreateInstance(CLSID_WICImagingFactory, nil,
      CLSCTX_INPROC_SERVER or CLSCTX_LOCAL_SERVER, IUnknown, FImagingFactory))
      or (FImagingFactory = nil) then
      raise ECannotCreateWICImagingFactory.CreateFmt
        (SCannotCreateWICImagingFactory, [ClassName]);
  end;
...
class function TCustomCanvasD2D.TryCreateDirect3DDevice: Boolean;
  ...
  begin
    ...
    begin
      ...
      Result := D3D10CreateDevice(nil, DriverType, 0, Flags, D3D10_SDK_VERSION,
        FSharedDevice);
...
begin
  if FDirect3DLevel < TDirect3DLevel.Direct3D10 then
    raise ECannotDetermineDirect3DLevel.CreateFmt(SCannotDetermineDirect3DLevel,
      [ClassName]);
...
{$IFDEF DXDEBUG}
  case FDirect3DSupport of
    ...
    TDirect3DSupport.Direct3D10:
      DebugText :=
        'Direct2D Canvas is using Direct3D 10.1 with 10.0 feature level';
...
begin
  if Succeeded(FSharedDevice.QueryInterface(IDXGIDevice, DXGIDevice)) and
    (DXGIDevice <> nil) and Succeeded(DXGIDevice.GetParent(IDXGIAdapter,
    DXGIAdapter)) and (DXGIAdapter <> nil) then
...
{$ELSE}
  FactoryOptions := nil;
{$ENDIF}
  Res := D2D1CreateFactory(D2D1_FACTORY_TYPE_MULTI_THREADED, ID2D1Factory,
    FactoryOptions, FSharedFactory);
  if Failed(Res) or (FSharedFactory = nil) then
    raise ECannotCreateD2DFactory.CreateFmt(SCannotCreateD2DFactory,
      [ClassName]);

  if Failed(DWriteCreateFactory(DWRITE_FACTORY_TYPE_SHARED, IDWriteFactory,
    IUnknown(FSharedDWriteFactory))) then
    raise ECannotCreateDWriteFactory.CreateFmt(SCannotCreateDWriteFactory,
      [ClassName]);
end;
...
begin
  W := 0;
  H := 0;
  ImagingFactory.CreateDecoderFromFilename(PChar(AFileName), GUID_NULL,
    $FFFFFFFF, WICDecodeMetadataCacheOnDemand, Decoder);
  if Decoder <> nil then
  begin
...
begin
  ...
  try
    ...
    try
      ...
      ImagingFactory.CreateDecoderFromStream(stream, GUID_NULL,
...
begin
  ...
  if (MaxSizeLimit > 0) and ((Width > MaxSizeLimit) or (Height > MaxSizeLimit))
  then
  begin
    ...
    if Succeeded(Scaler.Initialize(Frame, Trunc(R.Width), Trunc(R.Height),
      WICBitmapInterpolationModeLinear)) then
    begin
      ...
      begin
        ...
        Result := Succeeded(Converter.CopyPixels(nil, Bitmap.Pitch,
          Height * Cardinal(Bitmap.Pitch), PByte(Bitmap.Bits)));
      end;
    end;
  end
  else
...
begin
  ...
  try
    ...
    ImagingFactory.CreateDecoderFromStream(Adapter, GUID_NULL,
      WICDecodeMetadataCacheOnDemand, Decoder);
    if Decoder <> nil then
    begin
      Decoder.GetFrame(0, Frame);
...
begin
  ...
  begin
    ...
    begin
      ImagingFactory.CreateFormatConverter(Converter);
      if Succeeded(Converter.Initialize(Bmp, GUID_WICPixelFormat32bppPBGRA,
        WICBitmapDitherTypeNone, nil, 0, 0)) then
      begin
        Converter.GetSize(Width, Height);
...
function TBitmapCodecWIC.SaveToStream(const AStream: TStream;
  const Bitmap: TBitmapSurface; const Extension: string;
...
begin
  ...
  begin
    ...
    begin
      ...
      MaxSubstreamSize := 2 * Bitmap.BytesPerPixel * Bitmap.Width *
        Bitmap.Height + MaxBitmapHeaderSize;
      Stream.InitializeFromIStreamRegion(TStreamAdapter.Create(AStream)
        as IStream, ULARGE_INTEGER(InitPosition),
    ...
    begin
      ...
      begin
        Frame.WritePixels(Bitmap.Height, Bitmap.Pitch,
          Bitmap.Pitch * Bitmap.Height, PByte(Bitmap.Bits));
        Frame.Commit;
        Enc.Commit;
      end
      else
      begin
        ImagingFactory.CreateBitmapFromMemory(Bitmap.Width, Bitmap.Height,
          GUID_WICPixelFormat32bppPBGRA, Bitmap.Pitch,
          Bitmap.Pitch * Bitmap.Height, PByte(Bitmap.Bits), Bmp);
...
begin
  ...
  try
    Result := SaveToStream(Stream, Bitmap, ExtractFileExt(AFileName).ToLower,
      SaveParams);
  finally
    Stream.Free;
  end;
...
function BitmapProp(const PixelFormat: DXGI_FORMAT;
  const AlphaMode: TD2D1AlphaMode): TD2D1BitmapProperties;
begin
...
constructor TD2DBitmapHandle.Create(const AWidth, AHeight: Integer;
  const AAccess: TMapAccess);
begin
  inherited Create;
  FWidth := AWidth;
  FHeight := AHeight;
  FAccess := AAccess;
  FContextLostId := TMessageManager.DefaultManager.SubscribeToMessage
    (TContextLostMessage, ContextLostHandler);
end;
...
function TD2DBitmapHandle.CreateBitmap(const RenderTarget: ID2D1RenderTarget)
  : ID2D1Bitmap;
var
...
begin
  ...
  begin
    ...
    if FSharedBitmap = nil then
      TCanvasD2D.SharedRenderTarget.CreateSharedBitmap(IDXGISurface,
        Pointer(Texture as IDXGISurface), @Prop, FSharedBitmap);
    RenderTarget.CreateSharedBitmap(ID2D1Bitmap, Pointer(FSharedBitmap),
      @Prop, Result);
  end
  else
    Result := nil;
...
begin
  ...
  begin
    ...
    if Failed(TCustomCanvasD2D.SharedDevice.CreateTexture2D(Desc, nil, FTexture))
    then
...
procedure TD2DBitmapHandle.ContextLostHandler(const Sender: TObject;
  const Msg: TMessage);
begin
  FTexture := nil;
  FSharedBitmap := nil;
...
constructor TCanvasD2D.CreateFromWindow(const AParent: TWindowHandle;
...
begin
  ...
  if WindowHandleToPlatform(Parent).Transparency then
    WindowHandleToPlatform(Parent).CreateBuffer(WindowHandleToPlatform(Parent)
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        Prop := D2D1RenderTargetProperties(DefaultRenderTargetMode,
          D2D1PixelFormat(DXGI_FORMAT_UNKNOWN, D2D1_ALPHA_MODE_PREMULTIPLIED));

        if Failed(SharedFactory.CreateDxgiSurfaceRenderTarget
          (TD2DBitmapHandle(Bitmap.Handle).Texture as IDXGISurface, Prop,
          FTarget)) or (FTarget = nil) then
          raise ECannotCreateRenderTarget.CreateFmt(SCannotCreateRenderTarget,
            [ClassName]);
      end;
    end
    else if (Parent <> nil) then
    begin
      FBufferSize := TSize.Create(WindowHandleToPlatform(Parent)
        .WndClientSize.Width, WindowHandleToPlatform(Parent)
        .WndClientSize.Height);

      if not(SameValue(FBufferSize.Width, 0, TEpsilon.Position) or
        SameValue(FBufferSize.Height, 0, TEpsilon.Position)) then
      begin
        if WindowHandleToPlatform(Parent).Transparency then
        begin
          WindowHandleToPlatform(Parent).ResizeBuffer(FBufferSize.Width,
            FBufferSize.Height);
          ...
          Desc.SampleDesc.Quality := 0;
          Desc.Usage := D3D10_USAGE_DEFAULT;
          Desc.BindFlags := D3D10_BIND_RENDER_TARGET or
            D3D10_BIND_SHADER_RESOURCE;
          if Failed(SharedDevice.CreateTexture2D(Desc, nil, FBufferTexture)) or
            (FBufferTexture = nil) then
            raise ECannotCreateTexture.CreateFmt(SCannotCreateTexture,
              [ClassName]);

          Prop := D2D1RenderTargetProperties(DefaultRenderTargetMode,
            D2D1PixelFormat(DXGI_FORMAT_UNKNOWN,
            D2D1_ALPHA_MODE_PREMULTIPLIED));

          if Failed(SharedFactory.CreateDxgiSurfaceRenderTarget
            (FBufferTexture as IDXGISurface, Prop, FTarget)) or (FTarget = nil)
          then
            raise ECannotCreateRenderTarget.CreateFmt(SCannotCreateRenderTarget,
              [ClassName]);
          ...
          if Failed(SharedDevice.CreateTexture2D(Desc, nil, FCopyBuffer)) or
            (FCopyBuffer = nil) then
            raise ECannotCreateTexture.CreateFmt(SCannotCreateTexture,
              [ClassName]);
        end
        else
        begin
          ...
          if Failed(SharedDXGIFactory.CreateSwapChain(SharedDevice, SwapDesc,
            FSwapChain)) or (FSwapChain = nil) then
            raise ECannotCreateSwapChain.CreateFmt(SCannotCreateSwapChain,
              [ClassName]);

          if Failed(FSwapChain.GetBuffer(0, ID3D10Texture2D, BackBuffer)) or
            (BackBuffer = nil) then
            raise ERetrieveSurfaceContents.CreateFmt(SRetrieveSurfaceContents,
              [ClassName]);

          if Failed(SharedDevice.CreateRenderTargetView(BackBuffer, nil,
            @FRenderTargetView)) then
            raise ECannotCreateRenderTargetView.CreateFmt
              (SCannotCreateRenderTargetView, [ClassName]);
          ...
          if Failed(SharedFactory.CreateDxgiSurfaceRenderTarget
            (BackBuffer as IDXGISurface, Prop, FTarget)) then
            raise ECannotCreateRenderTarget.CreateFmt(SCannotCreateRenderTarget,
              [ClassName]);

          SharedDXGIFactory.MakeWindowAssociation(WindowHandleToPlatform(Parent)
            .Wnd, DXGI_MWA_NO_WINDOW_CHANGES);
        end;
      end;
    end;
    if FTarget <> nil then
...
function TCanvasD2D.CreateD2DGradientBrush(AGradient: TGradient;
  const ARect: TRectF; const AOpacity: Single): ID2D1Brush;
var
  Count: Integer;
...
begin
  ...
  begin
    ...
    begin
      Grad[Count].Color := D2Color(MakeColor(AGradient.Points[0].IntColor,
        AOpacity), 1);
    ...
    begin
      Grad[I + Count].Color := D2Color(MakeColor(AGradient.Points[I].IntColor,
        AOpacity), 1);
    ...
    begin
      ...
      Grad[AGradient.Points.Count + Count - 1].Color :=
        D2Color(MakeColor(AGradient.Points[AGradient.Points.Count - 1].IntColor,
        AOpacity), 1);
    ...
    begin
      { Linear }
      FTarget.CreateGradientStopCollection(@Grad[0], AGradient.Points.Count +
        Count, D2D1_GAMMA_2_2, D2D1_EXTEND_MODE_CLAMP, GradCol);
      GradBrushProp.StartPoint := D2Point(ARect.Left + AGradient.StartPosition.X
        * ARect.Width, ARect.Top + AGradient.StartPosition.Y * ARect.Height);
      GradBrushProp.EndPoint := D2Point(ARect.Left + AGradient.StopPosition.X *
        ARect.Width, ARect.Top + AGradient.StopPosition.Y * ARect.Height);
      FTarget.CreateLinearGradientBrush(GradBrushProp, nil, GradCol,
    ...
    begin
      ...
      FTarget.CreateGradientStopCollection(@Grad[0], AGradient.Points.Count +
        Count, D2D1_GAMMA_2_2, D2D1_EXTEND_MODE_CLAMP, GradCol);
      RadialGradBrushProp.GradientOriginOffset :=
        TD2D1Point2F(TPointF.Create(0, 0));
      RadialGradBrushProp.Center :=
        TD2D1Point2F(TPointF.Create(ARect.Width * 0.5, ARect.Height * 0.5));
      RadialGradBrushProp.RadiusX := ARect.Width / 2;
      RadialGradBrushProp.RadiusY := ARect.Height / 2;
      FTarget.CreateRadialGradientBrush(RadialGradBrushProp, nil, GradCol,
        ID2D1RadialGradientBrush(Result));
      UpdateBrushMatrix(Result, AGradient.RadialTransform.Matrix);
      GradCol := nil;
    end;
  end
  else
    FTarget.CreateSolidColorBrush(D2Color(0, 0), nil,
      ID2D1SolidColorBrush(Result));
end;

function TCanvasD2D.CreateD2DBitmapBrush(ABitmap: TBitmap;
  const AWrapMode: TWrapMode; const ARect: TRectF; const AOpacity: Single)
  : ID2D1Brush;
var
...
begin
  ...
  begin
    ...
    FTarget.CreateBitmapBrush(D2DBitmap, @BitmapBrushProp, @BrushProp,
      ID2D1BitmapBrush(Result));
  end
  else
    FTarget.CreateSolidColorBrush(D2Color(0, 0), nil,
      ID2D1SolidColorBrush(Result));
...
procedure TCanvasD2D.ContextLostHandler(const Sender: TObject;
  const Msg: TMessage);
begin
  DisposeResources;
end;

function TCanvasD2D.DoBeginScene(const AClipRects: PClipRects;
        ...
        try
          ...
          begin
            for I := 0 to FBufferSize.Height - 1 do
              Move(PAlphaColorArray(Mapped.pData)
                [Cardinal(I) * (Mapped.RowPitch div 4)],
                PAlphaColorArray(WindowHandleToPlatform(Parent).BufferBits)
                [I * FBufferSize.Width], FBufferSize.Width * 4);
          end
          else
            Move(Mapped.pData^, WindowHandleToPlatform(Parent).BufferBits^,
              FBufferSize.Width * FBufferSize.Height * 4);
        finally
          FCopyBuffer.Unmap(0);
        end;
...
begin
  TMessageManager.DefaultManager.SendMessage(nil,
    TContextLostMessage.Create, False);
  DestroySharedResources;
end;
...
begin
  ...
  begin
    if (Length(ARects) = 1) and (ARects[0] = TRectF.Create(0, 0, Width, Height))
    then
      Exit;
    ...
    begin
      ...
      if not IsScaleInteger then
        R := AlignToPixel(R);
      SharedFactory.CreateRectangleGeometry(D2Rect(R),
        ID2D1RectangleGeometry(Geoms[I]));
    end;
    SharedFactory.CreateGeometryGroup(D2D1_FILL_MODE_WINDING, @Geoms[0],
      Length(Geoms), FClipGeom);
    for I := 0 to High(ARects) do
      Geoms[I] := nil;
    // Apply clips
...
begin
  ...
  begin
    ...
    begin
      ...
      LayerPar.ContentBounds := D2Rect(RectF(-MaxSingle, -MaxSingle, MaxSingle,
        MaxSingle));
...
begin
  ...
  begin
    ...
    OleCheck(SharedFactory.CreateGeometryGroup(D2D1_FILL_MODE_WINDING,
      @Geoms[0], Length(Geoms), GeomGroup));
    if GeomGroup <> nil then
    begin
      FCurrentSaveState.CreateLayer(FTarget);
      LayerPar.ContentBounds :=
        D2Rect(RectF(-FWidth, -FHeight, 400000, 400000));
...
procedure TCanvasD2D.ApplyFill(const ABrush: TBrush; ARect: TRectF;
  const AOpacity: Single);
var
  BrushMatrix: TMatrix;
begin
  if (ABrush.Kind = TBrushKind.Resource) and (ABrush.Resource <> nil) and
    (ABrush.Resource.Brush <> nil) then
    ABrush.Assign(ABrush.Resource.Brush);

  if FMetaBrush = nil then
    FMetaBrush := TMetaBrush.Create;

  if FMetaBrush.Valid and ((FMetaBrush.Kind <> ABrush.Kind) or
    (not SameValue(FMetaBrush.Opacity, AOpacity, TEpsilon.Scale))) then
    FMetaBrush.Valid := False;
  ...
  case ABrush.Kind of
    TBrushKind.Solid:
      ...
      begin
        FTarget.CreateSolidColorBrush(D2Color(ABrush.Color, AOpacity), nil,
          ID2D1SolidColorBrush(FBrush));
        FMetaBrush.Color := ABrush.Color;
      end;

    TBrushKind.Gradient:
      if (FMetaBrush.Gradient.Style <> ABrush.Gradient.Style) or
        (not FMetaBrush.Valid) or (FMetaBrush.Rect <> ARect) or
      ...
      else if FBrush <> nil then
      begin
        BrushMatrix := ABrush.Gradient.RadialTransform.Matrix;
        if not CompareMem(@BrushMatrix, @FLastBrushTransform, SizeOf(TMatrix))
        then
        begin
    ...
    TBrushKind.Bitmap:
      if (not FMetaBrush.Valid) or (FMetaBrush.Image <> ABrush.Bitmap.Image) or
        (FMetaBrush.Rect <> ARect) or
        (FMetaBrush.WrapMode <> ABrush.Bitmap.WrapMode) then
      begin
        FBrush := CreateD2DBitmapBrush(ABrush.Bitmap.Bitmap,
          ABrush.Bitmap.WrapMode, ARect, AOpacity);
...
procedure TCanvasD2D.ApplyStroke(const AStroke: TStrokeBrush; ARect: TRectF;
  const AOpacity: Single);
var
  StyleProp: TD2D1StrokeStyleProperties;
begin
  if (AStroke.Kind = TBrushKind.Resource) and (AStroke.Resource <> nil) and
  ...
  case AStroke.Kind of
    ...
    TBrushKind.Bitmap:
      if (not FMetaStroke.Valid) or (FMetaStroke.Image <> AStroke.Bitmap.Image)
        or (FMetaStroke.Rect <> ARect) or
        (FMetaStroke.WrapMode <> AStroke.Bitmap.WrapMode) then
      begin
        FStrokeBrush := CreateD2DBitmapBrush(AStroke.Bitmap.Bitmap,
          AStroke.Bitmap.WrapMode, ARect, AOpacity);
    ...
    TBrushKind.Gradient:
      ...
      begin
        FStrokeBrush := CreateD2DGradientBrush(AStroke.Gradient, ARect,
          AOpacity);
        FMetaStroke.Rect := ARect;
  ...
  if (not FMetaStrokeBrush.Valid) or (FMetaStrokeBrush.Cap <> AStroke.Cap) or
    (FMetaStrokeBrush.Join <> AStroke.Join) or
    (FMetaStrokeBrush.Dash <> AStroke.Dash) or
    ((AStroke.Dash = TStrokeDash.Custom) and
    ((FMetaStrokeBrush.DashArray <> AStroke.DashArray) or
    not SameValue(FMetaStrokeBrush.DashOffset, AStroke.DashOffset,
    TEpsilon.Vector))) then
  begin
    case AStroke.Cap of
      TStrokeCap.Flat:
    ...
    if AStroke.Dash = TStrokeDash.Custom then
      SharedFactory.CreateStrokeStyle(StyleProp, @AStroke.DashArray[0],
        Length(AStroke.DashArray), FStrokeStyle)
    else
...
procedure TCanvasD2D.DoDrawLine(const APt1, APt2: TPointF;
  const AOpacity: Single; const ABrush: TStrokeBrush);
begin
  if FTarget <> nil then
  begin
    ApplyStroke(ABrush, TRectF.Create(APt1.X, APt1.y, APt2.X, APt2.y),
      AOpacity);
    FTarget.DrawLine(D2Point(APt1.X, APt1.y), D2Point(APt2.X, APt2.y),
      FStrokeBrush, ABrush.Thickness, FStrokeStyle);
  end;
end;

procedure TCanvasD2D.DoDrawRect(const ARect: TRectF; const AOpacity: Single;
  const ABrush: TStrokeBrush);
begin
  if FTarget <> nil then
  begin
    ApplyStroke(ABrush, ARect, AOpacity);
    FTarget.DrawRectangle(D2Rect(ARect), FStrokeBrush, ABrush.Thickness,
      FStrokeStyle);
  end;
end;

procedure TCanvasD2D.DoFillRect(const ARect: TRectF; const AOpacity: Single;
  const ABrush: TBrush);
begin
  if FTarget <> nil then
  begin
...
procedure TCanvasD2D.DoDrawEllipse(const ARect: TRectF; const AOpacity: Single;
  const ABrush: TStrokeBrush);
begin
  if FTarget <> nil then
  begin
    ApplyStroke(ABrush, ARect, AOpacity);
    FTarget.DrawEllipse(D2Ellipse(ARect), FStrokeBrush, ABrush.Thickness,
      FStrokeStyle);
  end;
end;

procedure TCanvasD2D.DoFillEllipse(const ARect: TRectF; const AOpacity: Single;
  const ABrush: TBrush);
begin
  if FTarget <> nil then
  begin
...
begin
  ...
  WS := FFont.Family;
  SharedDWriteFactory.CreateTextFormat(PChar(WS), nil,
    FontWeightToDWrite(FFont.StyleExt.Weight),
    FontSlantToDWrite(FFont.StyleExt.Slant),
    FontStretchToDWrite(FFont.StyleExt.Stretch), FFont.Size, PChar(FLocaleName),
    TextFormat);
  if TFillTextFlag.RightToLeft in Flags then
    TextFormat.SetReadingDirection(DWRITE_READING_DIRECTION_RIGHT_TO_LEFT);

  SharedDWriteFactory.CreateTextLayout(PChar(AText), Length(AText), TextFormat,
    RWidth, RHeight, TextLayout);
...
begin
  ...
  begin
    ...
    if Failed(SharedFactory.CreateDxgiSurfaceRenderTarget
      (FSharedTexture as IDXGISurface, Prop, FSharedRenderTarget)) then
      raise ECannotCreateRenderTarget.CreateFmt(SCannotCreateRenderTarget,
        [ClassName]);
  end;
end;
...
class function TCanvasD2D.DoInitializeBitmap(const Width, Height: Integer;
...
class function TCanvasD2D.DoMapBitmap(const Bitmap: THandle;
...
class procedure TCanvasD2D.DoUnmapBitmap(const Bitmap: THandle;
...
procedure TCanvasD2D.DoDrawBitmap(const ABitmap: TBitmap;
  const SrcRect, DstRect: TRectF; const AOpacity: Single;
...
procedure TCanvasD2D.DoDrawPath(const APath: TPathData; const AOpacity: Single;
  const ABrush: TStrokeBrush);
var
  I: Integer;
  CP1, CP2: TPointF;
...
begin
  ...
  begin
      ...
      case APath[I].Kind of
        TPathPointKind.MoveTo:
          begin
            ...
            Path.BeginFigure(D2Point(APath[I].Point.X, APath[I].Point.Y),
        ...
        TPathPointKind.CurveTo:
          begin
            ...
            CP2 := APath[I].Point;
            Inc(I);
            Path.AddBezier(D2Bezier(CP1.X, CP1.y, CP2.X, CP2.y,
    ...
    if not Closed then
      Path.EndFigure(D2D1_FIGURE_END_OPEN);
    Path.Close;
    FTarget.DrawGeometry(Geometry, FStrokeBrush, ABrush.Thickness,
      FStrokeStyle);
  end;
end;

procedure TCanvasD2D.DoFillPath(const APath: TPathData; const AOpacity: Single;
  const ABrush: TBrush);
var
  I: Integer;
  CP1, CP2: TPointF;
...
begin
    ...
    case APath[I].Kind of
      ...
      TPathPointKind.CurveTo:
        begin
          ...
          CP2 := APath[I].Point;
          Inc(I);
          Path.AddBezier(D2Bezier(CP1.X, CP1.Y, CP2.X, CP2.y, APath[I].Point.X,
            APath[I].Point.Y));
        end;
      TPathPointKind.Close:
        Path.EndFigure(D2D1_FIGURE_END_CLOSED);
  ...
  if (APath.Count < 1) or (APath[APath.Count - 1].Kind <> TPathPointKind.Close)
  then
    Path.EndFigure(D2D1_FIGURE_END_OPEN);
  Path.Close;
...
function TCanvasD2D.PtInPath(const APoint: TPointF;
...
begin
  ...
  begin
    ...
    if not Closed then
      Path.EndFigure(D2D1_FIGURE_END_OPEN);
    Path.Close;
    Geometry.FillContainsPoint(D2Point(APoint.X, APoint.Y),
      TD2DMatrix3x2F.Identity, 1, Cont);
    Result := Cont;
  end;
end;
...
procedure TSink.BeginFigure(StartPoint: TD2D1Point2F;
  FigureBegin: TD2D1FigureBegin);
begin
...
begin
  ...
  begin
    Path.CurveTo(TPointF.Create(Beziers.point1.x + ShiftX,
      Beziers.point1.y + ShiftY), TPointF.Create(Beziers.point2.x + ShiftX,
      Beziers.point2.y + ShiftY), TPointF.Create(Beziers.point3.x + ShiftX,
      Beziers.point3.y + ShiftY));
    Inc(Beziers);
  end;
end;
...
type
  ...
  private
    ...
    function DrawUnderline(clientDrawingContext: Pointer;
      baselineOriginX: Single; baselineOriginY: Single;
      var underline: TDwriteUnderline; const clientDrawingEffect: IUnknown)
      : HResult; stdcall;
    ...
    function DrawInlineObject(clientDrawingContext: Pointer; originX: Single;
      ...
      isRightToLeft: BOOL; const clientDrawingEffect: IUnknown)
      : HResult; stdcall;
  end;

function TTextRendering.DrawGlyphRun(clientDrawingContext: Pointer;
  baselineOriginX: Single; baselineOriginY: Single;
  measuringMode: TDWriteMeasuringMode; var glyphRun: TDwriteGlyphRun;
  var glyphRunDescription: TDwriteGlyphRunDescription;
...
begin
  ...
  if Succeeded(Res) then
    Res := glyphRun.fontFace.GetGlyphRunOutline(glyphRun.fontEmSize,
      glyphRun.glyphIndices, glyphRun.glyphAdvances, glyphRun.glyphOffsets,
      glyphRun.glyphCount, glyphRun.isSideways, False, Sink);
  if Succeeded(Res) then
    Sink.Close;
  ...
  PathGeometry.Simplify(D2D1_GEOMETRY_SIMPLIFICATION_OPTION_CUBICS_AND_LINES,
    TD2DMatrix3x2F.Identity, 0, SimSink);
  SimSink.Close;
  SimSink.Free;
  Result := S_OK;
end;
...
function TTextRendering.DrawStrikethrough(clientDrawingContext: Pointer;
  baselineOriginX: Single; baselineOriginY: Single;
  var strikethrough: TDwriteStrikethrough;
  const clientDrawingEffect: IUnknown): HResult;
begin
...
function TTextRendering.DrawUnderline(clientDrawingContext: Pointer;
  baselineOriginX: Single; baselineOriginY: Single;
  var underline: TDwriteUnderline; const clientDrawingEffect: IUnknown)
  : HResult;
begin
...
function TTextRendering.GetCurrentTransform(clientDrawingContext: Pointer;
...
function TTextRendering.GetPixelsPerDip(clientDrawingContext: Pointer;
...
function TTextRendering.IsPixelSnappingDisabled(clientDrawingContext: Pointer;
...
procedure TD2DCanvasSaveState.ContextLostHandler(const Sender: TObject;
  const Msg: TMessage);
begin
  FStateBlock := nil;
  FLayer := nil;
...
begin
  ...
  begin
    ...
    if Succeeded(TCanvasD2D.SharedFactory.CreateDrawingStateBlock(nil, nil,
      FStateBlock)) then
    begin
...
procedure TD2DCanvasSaveState.CreateLayer(const RenderTarget
  : ID2D1RenderTarget);
begin
...
class function TTextLayoutD2D.TDWriteFontDescriptor.Create(const AFamilyName
  : string; const AWeight: TDWriteFontWeight; const AStyle: TDWriteFontStyle;
  const AStretch: TDWriteFontStretch): TDWriteFontDescriptor;
begin
...
function TTextLayoutD2D.GetFontStyles(const AFont: TFont)
  : TDWriteFontDescriptor;

  function GetDWriteLocalizedName(const Names: IDWriteLocalizedStrings;
...
var
  ...
  LDC: HDC;
begin
  Result := TDWriteFontDescriptor.Create(AFont.Family,
    FontWeightToDWrite(AFont.StyleExt.Weight),
    FontSlantToDWrite(AFont.StyleExt.Slant),
    FontStretchToDWrite(AFont.StyleExt.Stretch));

  if FFontCollection = nil then
    TCanvasD2D.SharedDWriteFactory.GetSystemFontCollection(FFontCollection);
  FFontCollection.FindFamilyName(PChar(AFont.Family), FamilyIndex,
    FamilyExists);
  ...
  begin
    ...
    begin
      if Succeeded(TCanvasD2D.SharedDWriteFactory.GetGdiInterop
        (DWriteGdiInterop)) then
        try
          ZeroMemory(@LogFont, SizeOf(LogFont));
          Move(PChar(AFont.Family)^, LogFont.lfFaceName,
            Length(AFont.Family) * SizeOf(Char));
          LogFont.lfCharSet := DEFAULT_CHARSET;
          LDC := GetDC(0);
          try
            EnumFontFamiliesEx(LDC, LogFont, @EnumFontFamExProc,
              NativeInt(@LogFont), 0);
          finally
            ReleaseDC(0, LDC);
          end;
          if Succeeded(DWriteGdiInterop.CreateFontFromLOGFONT(LogFont,
            DWriteFont)) then
            try
              if Succeeded(DWriteFont.GetFontFamily(DWriteFamily)) then
                try
                  ...
                  begin
                    GetLocaleInfo(LOCALE_USER_DEFAULT, LOCALE_SNAME, LocaleName,
                      LOCALE_NAME_MAX_LENGTH);
                    FLocaleName := LocaleName;
                  end;
                  if Succeeded(DWriteFamily.GetFamilyNames(Names)) then
                    try
                      Result := TDWriteFontDescriptor.Create
                        (GetDWriteLocalizedName(Names, FLocaleName),
                        DWriteFont.GetWeight, DWriteFont.GetStyle,
                        DWriteFont.GetStretch);
...
begin
  Extra := Max(0, FMetrics.layoutWidth -
    FMetrics.widthIncludingTrailingWhitespace + FOverhangMetrics.right -
    FMetrics.left);
  ...
  FTextRect := TRectF.Create(FMetrics.left, FMetrics.top,
    FMetrics.left + LWidth, FMetrics.top + LHeight);
...
procedure TTextLayoutD2D.ContextLostHandler(const Sender: TObject;
  const Msg: TMessage);
begin
  FLayout := nil;
  FBrush := nil;
...
procedure TTextLayoutD2D.DoDrawLayout(const ACanvas: TCanvas);
  ...
  begin
    ...
    Result := SameValue(LColor.r, TAlphaColorRec(Color).R / $FF,
      TEpsilon.Vector) and SameValue(LColor.b, TAlphaColorRec(Color).B / $FF,
      TEpsilon.Vector) and SameValue(LColor.g, TAlphaColorRec(Color).G / $FF,
      TEpsilon.Vector) and SameValue(LColor.a, TAlphaColorRec(Color).A / $FF *
      Opacity, TEpsilon.Vector);
  end;

begin
  if (ACanvas = nil) or not(ACanvas is TCanvasD2D) or Text.IsEmpty or
    (FLayout = nil) then
    Exit;

  if (FLayout <> nil) and ((FBrush = nil) or not SameValue(FBrush.GetOpacity,
    Opacity, TEpsilon.Vector) or not SameColor) then
  begin
    FBrush := nil;
    TCanvasD2D(ACanvas).FTarget.CreateSolidColorBrush(D2Color(Color, Opacity),
      nil, ID2D1SolidColorBrush(FBrush));
  end;

  if (FLayout <> nil) and (FBrush <> nil) then
    TCanvasD2D(ACanvas).FTarget.DrawTextLayout(D2Point(TopLeft.X + Padding.Left,
      TopLeft.Y + Padding.Top), FLayout, FBrush, D2D1_DRAW_TEXT_OPTIONS_CLIP);
end;
...
begin
  ...
  if Succeeded(TCanvasD2D.SharedDWriteFactory.CreateTextFormat
    (PChar(FontDescriptor.FamilyName), FFontCollection, FontDescriptor.Weight,
    FontDescriptor.Style, FontDescriptor.Stretch, Font.Size, PChar(FLocaleName),
    TextFormat)) then
    try
      if RightToLeft then
        TextFormat.SetReadingDirection(DWRITE_READING_DIRECTION_RIGHT_TO_LEFT)
      ...
      if Trimming <> TTextTrimming.None then
        TCanvasD2D.SharedDWriteFactory.CreateEllipsisTrimmingSign(TextFormat,
          TrimmingSign);
      TextFormat.SetTrimming(TrimOptions, TrimmingSign);
      TrimmingSign := nil;

      if Succeeded(TCanvasD2D.SharedDWriteFactory.CreateTextLayout(PChar(Text),
        Text.Length, TextFormat, MaxSize.X - Padding.Left - Padding.Right,
        MaxSize.Y - Padding.Top - Padding.Bottom, FLayout)) then
      begin
        TextRange.StartPosition := 0;
        TextRange.Length := Text.Length;
        FLayout.SetStrikethrough(TFontStyle.fsStrikeOut
          in Font.StyleExt.SimpleStyle, TextRange);
        FLayout.SetUnderline(TFontStyle.fsUnderline
          in Font.StyleExt.SimpleStyle, TextRange);
        ...
        begin
          ...
          begin
            FLayout.SetStrikethrough(TFontStyle.fsStrikeOut
              in Attr.Attribute.Font.StyleExt.SimpleStyle, TextRange);
            FLayout.SetUnderline(TFontStyle.fsUnderline
              in Attr.Attribute.Font.StyleExt.SimpleStyle, TextRange);
            if not SameValue(Attr.Attribute.Font.Size, Font.Size,
            ...
            FLayout.SetFontFamilyName(PChar(FontDescriptor.FamilyName),
              TextRange);
          end;
          if (Attr.Attribute.Color <> TAlphaColorRec.Null) and
            Succeeded(TCanvasD2D.SharedRenderTarget.CreateSolidColorBrush
            (D2Color(Attr.Attribute.Color, Opacity), nil, SolidBrush)) then
...
class function TTextLayoutD2D.EnumFontFamExProc(const lpelfe: TLogFont;
  const lpntme: TNewTextMetric; FontType: DWORD; lParam: LPARAM): Integer;
var
  LFont: PLogFont;
begin
...
begin
  ...
  begin
    ...
    begin
      ...
      FLayout.HitTestTextRange(Result, 1, 0, 0, HitTestMetricsArray[0],
        Count, Count);
      if Count > 0 then
      begin
        if (HitTestMetricsArray[0].width > 0) and
          (APoint.X > (TopLeft.X + HitTestMetricsArray[0].Left +
          HitTestMetricsArray[0].width * 3 / 5)) then
          Inc(Result);
      end
      else if (FMetrics.widthIncludingTrailingWhitespace > 0) and
        (APoint.X > (TopLeft.X + FMetrics.left +
        FMetrics.widthIncludingTrailingWhitespace * 3 / 5)) then
        Inc(Result);
    end;
  end
  else if ((APoint.X < LRect.Left) or SameValue(APoint.X, LRect.Left,
    TEpsilon.Position)) and ((APoint.Y > LRect.Top) or SameValue(APoint.Y,
    LRect.Top, TEpsilon.Position)) and
    ((APoint.Y < LRect.Bottom) or SameValue(APoint.Y, LRect.Bottom,
    TEpsilon.Position)) then
    Result := 0;
  if (Result >= 0) and (Result < Text.Length) and Text.Chars[Result].IsLowSurrogate
  then
    Inc(Result);
end;
...
begin
  ...
  FLayout.HitTestTextRange(LPos, RemainsLength, 0, 0, HitTestMetrics[0],
    0, Count);
  ...
  if Count > 0 then
    ...
    begin
      ...
      Result[High(Result)] := TRectF.Create(HitTestMetric.left,
        HitTestMetric.top, HitTestMetric.left + HitTestMetric.width,
...
procedure RegisterCanvasClasses;
begin
  if GlobalUseDirect2D and (TCustomCanvasD2D.Direct3DLevel >=
    TCustomCanvasD2D.TDirect3DLevel.Direct3D10) and InitThemeLibrary and
    UseThemes and TCustomCanvasD2D.TryCreateDirect3DDevice then
  begin
...
TBitmapCodecManager.RegisterBitmapCodecClass(SBMPImageExtension, SVBitmaps,
  True, TBitmapCodecWIC);
TBitmapCodecManager.RegisterBitmapCodecClass(SJPGImageExtension, SVJPGImages,
  True, TBitmapCodecWIC);
TBitmapCodecManager.RegisterBitmapCodecClass(SJPEGImageExtension, SVJPGImages,
  True, TBitmapCodecWIC);
TBitmapCodecManager.RegisterBitmapCodecClass(SPNGImageExtension, SVPNGImages,
  True, TBitmapCodecWIC);
TBitmapCodecManager.RegisterBitmapCodecClass(SGIFImageExtension, SVGIFImages,
  True, TBitmapCodecWIC);
TBitmapCodecManager.RegisterBitmapCodecClass(STIFImageExtension, SVTIFFImages,
  True, TBitmapCodecWIC);
TBitmapCodecManager.RegisterBitmapCodecClass(STIFFImageExtension, SVTIFFImages,
  True, TBitmapCodecWIC);
TBitmapCodecManager.RegisterBitmapCodecClass(SICOImageExtension, SVIcons, True,
  TBitmapCodecWIC);
TBitmapCodecManager.RegisterBitmapCodecClass(SHDPImageExtension, SWMPImages,
  True, TBitmapCodecWIC);
