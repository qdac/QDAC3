...
begin
  ...
  {$IF RTLVersion>=26}
  AnsiStrings.
  {$IFEND}StrLFmt(PAnsiChar(Result), 38,'{%.8x-%.4x-%.4x-%.2x%.2x-%.2x%.2x%.2x%.2x%.2x%.2x}',   // do not localize
      ...
      begin
        ...
        begin
          case SysLocale.PriLangID of
            LANG_JAPANESE:
              Result := ALCopyStr(Result, 1, {$IF RTLVersion>=26}AnsiStrings.{$IFEND}CharToBytelen(Result, 1));
            LANG_CHINESE:
              if (SysLocale.SubLangID = SUBLANG_CHINESE_TRADITIONAL)
                and ({$IF RTLVersion>=26}AnsiStrings.{$IFEND}ByteToCharLen(Result, Length(Result)) = 4) then
              begin
                P := Buffer + {$IF RTLVersion>=26}AnsiStrings.{$IFEND}CharToByteIndex(Result, 3) - 1;
                SetString(Result, P, {$IF RTLVersion>=26}AnsiStrings.{$IFEND}CharToByteLen(P, 2));
              end;
          end;
        end;
      end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        //  Continue;
        //end;
        ALFormat := {$IF RTLVersion>=26}AnsiStrings.{$IFEND}StrNextChar(ALFormat);
        Token := Starter;
        ...
        case Token of
              ...
              begin
                ...
                case P^ of
                  'A', 'a':
                    ...
                    begin
                      if ( ({$IF RTLVersion>=26}AnsiStrings.{$IFEND}StrLIComp(P, 'AM/PM', 5) = 0)
                        or ({$IF RTLVersion>=26}AnsiStrings.{$IFEND}StrLIComp(P, 'A/P',   3) = 0)
                        or ({$IF RTLVersion>=26}AnsiStrings.{$IFEND}StrLIComp(P, 'AMPM',  4) = 0) ) then
                        Use12HourClock := True;
                      Break;
                    end;
                  'H', 'h':
          ...
          'A':
            begin
              GetTime;
              P := ALFormat - 1;
              if {$IF RTLVersion>=26}AnsiStrings.{$IFEND}StrLIComp(P, 'AM/PM', 5) = 0 then
              begin
              ...
              if {$IF RTLVersion>=26}AnsiStrings.{$IFEND}StrLIComp(P, 'A/P', 3) = 0 then
              begin
              ...
              if {$IF RTLVersion>=26}AnsiStrings.{$IFEND}StrLIComp(P, 'AMPM', 4) = 0 then
              begin
                if Hour < 12 then
              ...
              if {$IF RTLVersion>=26}AnsiStrings.{$IFEND}StrLIComp(P, 'AAAA', 4) = 0 then
              begin
                GetDate;
                AppendString(AFormatSettings.LongDayNames[DayOfWeek(DateTime)]);
                Inc(ALFormat, 3);
              end else
              if {$IF RTLVersion>=26}AnsiStrings.{$IFEND}StrLIComp(P, 'AAA', 3) = 0 then
              begin
                GetDate;
...
begin
  {$IFDEF UNICODE}
  Result := {$IF RTLVersion>=26}System.AnsiStrings.{$IFEND}PosEx(SubStr, S, Offset);
  {$ELSE}
...
begin
  {$IFDEF UNICODE}
  Result := {$IF RTLVersion>=26}System.AnsiStrings.{$IFEND}UpperCase(S);
  {$ELSE}
  Result := UpperCase(S);
  {$ENDIF}
end;
...
begin
  {$IFDEF UNICODE}
  Result := {$IF RTLVersion>=26}System.AnsiStrings.{$IFEND}LowerCase(S);
  {$ELSE}
  Result := LowerCase(S);
  {$ENDIF}
end;
...
begin
  {$IFDEF UNICODE}
  Result := {$IF RTLVersion>=26}System.AnsiStrings.{$IFEND}CompareStr(S1, S2);
  {$ELSE}
...
begin
  {$IFDEF UNICODE}
  Result := {$IF RTLVersion>=26}System.AnsiStrings.{$IFEND}SameStr(S1, S2);
  {$ELSE}
...
Begin
  {$IFDEF UNICODE}
  Result := {$IF RTLVersion>=26}System.AnsiStrings.{$IFEND}CompareText(S1, S2);
  {$ELSE}
...
begin
  {$IFDEF UNICODE}
  Result := {$IF RTLVersion>=26}System.AnsiStrings.{$IFEND}SameText(S1, S2);
  {$ELSE}
...
begin
  {$IFDEF UNICODE}
  Result := {$IF RTLVersion>=26}System.AnsiStrings.{$IFEND}Trim(S);
  {$ELSE}
  Result := Trim(S);
  {$ENDIF}
end;
...
begin
  {$IFDEF UNICODE}
  Result := {$IF RTLVersion>=26}System.AnsiStrings.{$IFEND}TrimLeft(S);
  {$ELSE}
  Result := TrimLeft(S);
  {$ENDIF}
end;
...
begin
  {$IFDEF UNICODE}
  Result := {$IF RTLVersion>=26}System.AnsiStrings.{$IFEND}TrimRight(S);
  {$ELSE}
  Result := TrimRight(S);
  {$ENDIF}
end;
...
begin
  ...
  DropCount := 1;
  P := Src;
  Src := {$IF RTLVersion>=26}AnsiStrings.{$IFEND}StrScan(Src, Quote);
  ...
  if Src = nil then Src := {$IF RTLVersion>=26}AnsiStrings.{$IFEND}StrEnd(P);
  ...
  begin
    ...
    Src := {$IF RTLVersion>=26}AnsiStrings.{$IFEND}StrScan(P, Quote);
    while Src <> nil do
    begin
      Inc(Src);
      if Src^ <> Quote then Break;
...
begin
  {$IFDEF UNICODE}
  Result := {$IF RTLVersion>=26}System.AnsiStrings.{$IFEND}ExtractFilePath(FileName);
  {$ELSE}
...
begin
  {$IFDEF UNICODE}
  Result := {$IF RTLVersion>=26}System.AnsiStrings.{$IFEND}ExtractFileDir(FileName);
  {$ELSE}
...
begin
  {$IFDEF UNICODE}
  Result := {$IF RTLVersion>=26}System.AnsiStrings.{$IFEND}ExtractFileDrive(FileName);
  {$ELSE}
...
begin
  {$IFDEF UNICODE}
  Result := {$IF RTLVersion>=26}System.AnsiStrings.{$IFEND}ExtractFileName(FileName);
  {$ELSE}
...
begin
  {$IFDEF UNICODE}
  Result := {$IF RTLVersion>=26}System.AnsiStrings.{$IFEND}ExtractFileExt(FileName);
  {$ELSE}
...
begin
  ...
  while Result > 0 do
  begin
    if (S[Result] <> #0) and ({$IF RTLVersion>=26}AnsiStrings.{$IFEND}StrScan(P, S[Result]) <> nil) then
      Exit;
    Dec(Result);
  end;
end;
