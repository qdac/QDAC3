...
type
  ...
  private
    ...
    m_Flags: Integer;
    m_stLock: TRTLCriticalSection;
  private
    function AddNewListPage(nNodeSize: UIntPtr): BOOL;
...
begin
  repeat
    Result := Dest;
    i := Result and AMask;
  until {$IFNDEF NextGen}InterlockedCompareExchange{$ELSE}AtomicCmpExchange{$ENDIF}(Dest,i,Result)=Result;
end;
...
begin
  ...
  //while ((AtomicOr(m_Flags, $01) and $01) <> 0) do
  //begin
    {$IFDEF QDAC_UNICODE}
    //TThread.Yield;
    {$ELSE}
...
begin
  ...
  //AtomicAnd(m_Flags, Integer($FFFFFFFE));
end;
...
function TSwzSingleListWithLock.AddNewListPage(nNodeSize: UIntPtr): BOOL;
var
  nPageSize: UIntPtr;
  lpListHeader: PSwzSingleListHeader;
begin
...
function TSwzSingleListWithLock.Push(p: Pointer): BOOL;
label
